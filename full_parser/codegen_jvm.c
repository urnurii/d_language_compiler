...
static int EmitLoadLValue(jvmc_class *cls, jvmc_code *code, NExpr *lhs, NParamList *params,
                          NStmt *body, SemanticContext *ctx, NType **type_out);
...
static int EmitSuperCtorCall(jvmc_class *cls, jvmc_code *code, NClassDef *class_def,
    ...
    if (base_info == NULL) {
        CodegenReportFail("EmitSuperCtorCall: base class info missing");
        return 0;
    }
    ...
    if (base_info->constructor_count > 0) {
        ...
        if (match == NULL || match->params == NULL) {
            CodegenReportFail("EmitSuperCtorCall: no matching base ctor");
            return 0;
        }
        if (match->params->count != arg_count) {
            CodegenReportFail("EmitSuperCtorCall: arg count mismatch");
            return 0;
        }
        owned_desc = BuildJvmMethodDescriptor(NULL, match->params);
        if (owned_desc == NULL) {
            CodegenReportFail("EmitSuperCtorCall: build descriptor failed");
            return 0;
        }
    ...
    if (base_internal == NULL) {
        CodegenReportFail("EmitSuperCtorCall: build base internal name failed");
        free(owned_desc);
        return 0;
    }
    ...
    free(base_internal);
    if (mref == NULL) {
        CodegenReportFail("EmitSuperCtorCall: methodref failed");
        free(ref_slots);
        free(owned_desc);
        return 0;
    }
    if (!jvmc_code_invokespecial(code, mref)) {
        CodegenReportFail("EmitSuperCtorCall: invokespecial failed");
        free(ref_slots);
        free(owned_desc);
        return 0;
    }
...
static int EmitThisCtorCall(jvmc_class *cls, jvmc_code *code, NClassDef *class_def,
    ...
    if (class_info == NULL || class_info->constructor_count <= 0) {
        CodegenReportFail("EmitThisCtorCall: class info missing");
        return 0;
    }
...

    {
        ...
        if (match == NULL || match->params == NULL) {
            CodegenReportFail("EmitThisCtorCall: no matching ctor");
            return 0;
        }
        if (match->params->count != arg_count) {
            CodegenReportFail("EmitThisCtorCall: arg count mismatch");
            return 0;
        }
        owned_desc = BuildJvmMethodDescriptor(NULL, match->params);
        if (owned_desc == NULL) {
            CodegenReportFail("EmitThisCtorCall: build descriptor failed");
            return 0;
        }
        ctor_desc = owned_desc;
    }
    ...
    if (owner_internal == NULL) {
        CodegenReportFail("EmitThisCtorCall: build owner internal name failed");
        free(owned_desc);
        return 0;
    }
    ...
    free(owner_internal);
    if (mref == NULL) {
        CodegenReportFail("EmitThisCtorCall: methodref failed");
        free(ref_slots);
        free(owned_desc);
        return 0;
    }
    if (!jvmc_code_invokespecial(code, mref)) {
        CodegenReportFail("EmitThisCtorCall: invokespecial failed");
        free(ref_slots);
        free(owned_desc);
        return 0;
    }
...
static int EmitAppendArrayElement(jvmc_class *cls, jvmc_code *code, NExpr *array_expr, NExpr *value_expr,
    ...
    jvmc_label *label_arr_null = jvmc_code_label_create(code);
    jvmc_label *label_len_done = jvmc_code_label_create(code);

    if (label_loop == NULL || label_done == NULL || label_arr_null == NULL || label_len_done == NULL) {
        return 0;
    }
    ...
    if (!jvmc_code_if_null(code, label_arr_null)) {
        return 0;
    }
    if (!jvmc_code_load_ref(code, (uint16_t)slot_arr)) {
        return 0;
    ...
    if (!jvmc_code_goto(code, label_len_done)) {
        return 0;
    }
    if (!jvmc_code_label_place(code, label_arr_null)) {
        return 0;
    }
    if (!jvmc_code_push_int(code, 0)) {
        return 0;
    ...
    if (!jvmc_code_label_place(code, label_len_done)) {
        return 0;
    }
...
static int EmitAppendArrayConcat(jvmc_class *cls, jvmc_code *code, NExpr *array_expr, NExpr *rhs_expr,
    ...
    jvmc_label *label_arr_null = jvmc_code_label_create(code);
    jvmc_label *label_len1_done = jvmc_code_label_create(code);
    jvmc_label *label_rhs_null = jvmc_code_label_create(code);
    jvmc_label *label_len2_done = jvmc_code_label_create(code);

    if (label_loop1 == NULL || label_done1 == NULL || label_loop2 == NULL || label_done2 == NULL ||
        label_arr_null == NULL || label_len1_done == NULL || label_rhs_null == NULL || label_len2_done == NULL) {
        return 0;
    }
    if (!GetElementTypeFromArray(array_expr ? array_expr->inferred_type : NULL, &elem_type)) {
        return 0;
    ...
    if (!jvmc_code_goto(code, label_len1_done)) {
        return 0;
    }
    if (!jvmc_code_label_place(code, label_arr_null)) {
        return 0;
    ...
    if (!jvmc_code_label_place(code, label_len1_done)) {
        return 0;
    }
    if (!jvmc_code_load_ref(code, (uint16_t)slot_rhs)) {
        return 0;
    }
    if (!jvmc_code_if_null(code, label_rhs_null)) {
        return 0;
    }
    if (!jvmc_code_load_ref(code, (uint16_t)slot_rhs)) {
        return 0;
    ...
    if (!jvmc_code_goto(code, label_len2_done)) {
        return 0;
    }
    if (!jvmc_code_label_place(code, label_rhs_null)) {
        return 0;
    }
    if (!jvmc_code_push_int(code, 0)) {
        return 0;
    ...
    if (!jvmc_code_label_place(code, label_len2_done)) {
        return 0;
    }
        ...
        case EXPR_FUNC_CALL: {
            ...
            }
            {
                ...
                if (ctx != NULL && ctx->current_class != NULL && expr->resolved_owner_internal != NULL) {
                    if (strcmp(expr->resolved_owner_internal, "Main") != 0) {
                        is_instance_call = 1;
                    }
                }
                if (is_instance_call) {
                ...
                if (!EmitInvokeFromRefKey(cls, code, &expr->jvm_ref_key, is_instance_call ? 0 : 1)) {
                    free(ref_slots);
                    return 0;
                }
