...
// ----- Функции для визуальных выражений
int ExprToDoT(FILE *file, NExpr *expr) {
    ...
    case EXPR_UNARY_OP: {
        fprintf(file, "  node_%d [label=\"op: %s\", shape=plaintext];\n",
            GetNextNodeId(), OpTypeToString(expr->value.unary.op));
...
// ----- Функции для визуализации типов -----
...
// ----- Функции для визуализации операторов
...
// Функция для визуализации программы
