...
static const char* StmtTypeToString(StmtType type) {
    switch (type) {
        ...
        case STMT_THIS_CTOR_CALL: return "this_ctor";
...
static long VisualizeStmt(NStmt *stmt) {
    ...
    switch (stmt->type) {
        ...
        case STMT_THIS_CTOR_CALL: {
            if (stmt->value.this_ctor.args) {
                for (int i = 0; i < stmt->value.this_ctor.args->count; i++) {
                    long arg_id = VisualizeExpr(stmt->value.this_ctor.args->elements[i]);
                    if (arg_id >= 0) {
                        DotPrintf("    node_%ld -> node_%ld [label=\"arg_%d\"];\n",
                                  node_id, arg_id, i);
                    }
            ...
            if (stmt->value.for_stmt.init_expr) {
                ...
                if (stmt->value.for_stmt.init_decls) {
                    for (int i = 0; i < stmt->value.for_stmt.init_decls->count; i++) {
                        NInitDecl *init_decl = stmt->value.for_stmt.init_decls->decls[i];
                        long var_id = GenerateNodeId();
                        DotPrintf("    node_%ld [label=\"var: %s\"];\n", var_id,
                                 EscapeString(init_decl->name));
                        DotPrintf("    node_%ld -> node_%ld [label=\"init_decl_%d\"];\n",
                                 node_id, var_id, i);
                ...
                case SOURCE_ITEM_DECL: {
                    ...
                    if (item->value.decl.init_decls) {
                        for (int i = 0; i < item->value.decl.init_decls->count; i++) {
                            NInitDecl *init_decl = item->value.decl.init_decls->decls[i];
                            long var_id = GenerateNodeId();
                            DotPrintf("    node_%ld [label=\"var: %s\"];\n", var_id,
                                     EscapeString(init_decl->name));
                            DotPrintf("    node_%ld -> node_%ld [label=\"var_%d\"];\n", decl_id, var_id, i);
